...
public class DefaultCxfBusConfigurer implements CxfBusConfigurer {
  ...
  private void configureLogs(Bus bus) {
    ...
    LOGGING_IN_INTERCEPTORS.forEach(inInterceptors::add);
    LOGGING_OUT_INTERCEPTORS.forEach(outInterceptors::add);

    LOGGING_IN_INTERCEPTORS1.forEach(inFaultInterceptors::add);
    LOGGING_OUT_INTERCEPTORS1.forEach(outFaultInterceptors::add);
  }
